...
| Host I/O smoke | Save file, injected Maple A, AI ring DMA |
...
Phase 3  I/O wired into emulator     (host smokes done; AICA/Maple on hardware next)
...
- [x] Host CPU test ROM **DC CPUTEST** (ORI/ADDU/XOR/ANDI/SLL/LUI/SW/LW + BEQ spin)
...
- [ ] PIF/SI controller stream from a booting game
...
Dummy ROM: IPL `B -1`. CPUTEST: IPL ALU + store/load to RDRAM `0x80000000`. Generated by `roms/gen_dc_roms.py`.
...
| `roms/dc_cputest.z64` | Interpreter ALU + RDRAM SW/LW |
...
The portable core is real. Host bring-up now **loads a ROM, retires MIPS ALU/LS, talks to the save backend, Maple inject, and AI ring**. Next is KallistiOS hardware and a software first frame.
